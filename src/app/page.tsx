...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen min-w-[1280px] overflow-x-hidden font-best text-ink">
      ...
      <div className={showSite ? "snap-y snap-mandatory" : "h-screen overflow-hidden"}>
        <TopSection />
        <AboutSection
      ...
      <AnimatePresence>
        ...
        {openingStage === "conversation" ? (
          <motion.div
            animate={{ opacity: 1 }}
            className="fixed inset-0 z-40 flex min-w-[1280px] items-end justify-center bg-white/80 px-20 pb-20 font-best"
            exit={{ opacity: 0 }}
            initial={{ opacity: 0 }}
          >
...
function CharacterTalkWindow({
  ...
  return (
    <div
      className="fixed z-40 w-[430px] max-w-[calc(100vw-80px)] font-best"
      style={{ left: "max(40px, calc(50% - 520px))", top: "max(92px, calc(75vh - 500px))" }}
    >
      <div
        className="pixel-panel min-h-[160px] px-7 py-6 text-left text-xl leading-[1.65]"
        ...
        {state.mode === "question" ? (
          <div className="mt-5 grid gap-3">
            {state.options.map((option, index) => {
              ...
              return (
                <button
                  className={`block w-full text-left text-xl ${selected ? "text-rose" : "text-ink"}`}
                  key={option.label}
  ...
  return (
    <section className="section-shell" id="top">
      <div className="section-grid">
        <div className="pixel-panel relative px-9 py-8">
          <CardDecoration
            className="-left-9 -top-9 z-0 w-[116px] rotate-[-10deg]"
            src="/decorations/ribbon_pink_01.png"
          />
          <p className="mb-4 text-3xl">sorano portfolio</p>
          <p className="text-xl leading-loose">Software, hardware, design and pixel art.</p>
        </div>
        <div aria-hidden />
        <div className="pixel-panel px-9 py-8">
          <p className="mb-5 text-2xl">WELCOME</p>
          <p className="text-lg leading-loose">Scroll to explore works, news and contact links.</p>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="section-shell" id="about" ref={sectionRef}>
      <div className="section-grid">
        <div>
          {hasSeenAbout ? (
            <div className="pixel-panel px-8 py-7 text-xl leading-loose">
              <p>ABOUT</p>
...
function ProfileCard() {
  return (
    <div className="pixel-panel relative px-9 py-8">
      <CardDecoration
        className="-right-10 -top-10 z-0 w-[112px] rotate-[10deg]"
        src="/decorations/ribbon_pink_01.png"
      />
      <p className="mb-8 text-3xl">PROFILE</p>
  ...
  return (
    <section className="section-shell" id="news">
      <div className="section-grid">
          ...
          {news.map((item, index) => {
            const content = (
              <>
                {index === 1 ? (
                  <CardDecoration
                    className="-left-6 -top-7 z-0 w-[64px] rotate-[-13deg]"
                    src="/decorations/ribbon_pink_02.png"
                  />
                ) : null}
                <div className="mb-3 flex items-start justify-between gap-4">
                  <p className="text-sm text-rose">{item.date}</p>
            ...
            return item.url ? (
              <a
                className="pixel-panel relative block px-7 py-6 transition hover:-translate-y-1 hover:border-[4px] hover:shadow-lift"
                href={item.url}
                key={item.id}
              ...
              <article className="pixel-panel relative px-7 py-6" key={item.id}>
                {content}
              </article>
            );
          })}
        ...
        <div className="pixel-panel relative px-9 py-8">
          <CardDecoration
            className="-right-8 -top-9 z-0 w-[80px] rotate-[15deg]"
            src="/decorations/mark_heart_pink_03.png"
          />
          <p className="mb-5 text-3xl">NEWS</p>
          <p className="text-xl leading-loose">Updates are listed from newest to oldest.</p>
        </div>
      </div>
    </section>
  );
...
function WorksSection() {
  ...
  return (
    <section className="section-shell" id="works">
      <div className="grid min-h-[calc(100vh-220px)] grid-cols-[1fr_420px_1fr] gap-12">
        {categories.map((category, index) => (
          <div className={index === 1 ? "col-start-3" : ""} key={category}>
            <p className="mb-5 text-2xl">{category}</p>
...
function WorkCard({ work }: { work: (typeof works)[number] }) {
  const decoration =
    work.id === "pixel-room"
      ? {
          className: "-left-4 -top-5 z-0 w-[58px] rotate-[-10deg]",
          src: "/decorations/ribbon_pink_02.png" as const
        }
      : work.id === "tiny-device"
        ? {
            className: "-right-5 -top-6 z-0 w-[54px] rotate-[12deg]",
  ...
  const inner = (
    <>
      ...
      <div className="grid grid-cols-[104px_1fr] gap-5">
        <div className="flex aspect-square items-center justify-center border-[3px] border-line bg-soft text-center text-xs">
          {work.thumbnail ? (
        ...
        <div>
          ...
          <p className="mb-3 text-sm leading-relaxed">{work.description}</p>
  ...
  const className =
    "pixel-panel relative block px-5 py-5 transition hover:-translate-y-1 hover:border-[4px] hover:shadow-lift";
  ...
  return (
    <section className="section-shell" id="contact">
      <div className="section-grid">
        <div className="pixel-panel relative px-9 py-8">
          <CardDecoration
            className="-left-8 -top-9 z-0 w-[102px] rotate-[-9deg]"
            src="/decorations/ribbon_pink_02.png"
          />
          <p className="mb-5 text-3xl">CONTACT</p>
          <p className="text-xl leading-loose">Please use the available links below.</p>
        </div>
        <div aria-hidden />
        <div className="grid gap-5">
          {contacts.map((contact) => {
            ...
            return contact.enabled && href ? (
              <a
                className="pixel-button block px-8 py-5 text-center text-2xl"
                href={href}
                key={contact.label}
                rel="noopener noreferrer"
                target="_blank"
              ...
              <button className="pixel-button px-8 py-5 text-center text-2xl" disabled key={contact.label} type="button">
                {contact.label}
              </button>
            );
          })}
